...
/**
 * REST controller for managing {@link ar.teco.domain.Segmento}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class SegmentoResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/segmentos/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Segmento> partialUpdateSegmento(
        ...
        Optional<Segmento> result = segmentoRepository
            ...
            .map(existingSegmento -> {
                if (segmento.getCountryName() != null) {
                    existingSegmento.setCountryName(segmento.getCountryName());
                }
