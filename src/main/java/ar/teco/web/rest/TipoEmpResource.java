...
/**
 * REST controller for managing {@link ar.teco.domain.TipoEmp}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class TipoEmpResource {
    ...
    public ResponseEntity<TipoEmp> updateTipoEmp(@PathVariable(value = "id", required = false) final Long id, @RequestBody TipoEmp tipoEmp)
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/tipo-emps/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<TipoEmp> partialUpdateTipoEmp(
        @PathVariable(value = "id", required = false) final Long id,
        @RequestBody TipoEmp tipoEmp
    ) throws URISyntaxException {
        ...
        Optional<TipoEmp> result = tipoEmpRepository
            ...
            .map(existingTipoEmp -> {
                if (tipoEmp.getDemanda() != null) {
                    existingTipoEmp.setDemanda(tipoEmp.getDemanda());
                }
