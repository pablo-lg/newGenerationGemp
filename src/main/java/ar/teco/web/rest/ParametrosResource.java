...
/**
 * REST controller for managing {@link ar.teco.domain.Parametros}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ParametrosResource {
    ...
    public ResponseEntity<Parametros> partialUpdateParametros(
        ...
        Optional<Parametros> result = parametrosRepository
            ...
            .map(existingParametros -> {
                if (parametros.getClave() != null) {
                    existingParametros.setClave(parametros.getClave());
                }
                if (parametros.getValor() != null) {
                    existingParametros.setValor(parametros.getValor());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, parametros.getId().toString())
        );
    }
