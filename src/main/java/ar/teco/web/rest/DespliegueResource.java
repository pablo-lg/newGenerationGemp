...
/**
 * REST controller for managing {@link ar.teco.domain.Despliegue}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class DespliegueResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/despliegues/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Despliegue> partialUpdateDespliegue(
        ...
        Optional<Despliegue> result = despliegueRepository
            ...
            .map(existingDespliegue -> {
                if (despliegue.getRegionName() != null) {
                    existingDespliegue.setRegionName(despliegue.getRegionName());
                }
